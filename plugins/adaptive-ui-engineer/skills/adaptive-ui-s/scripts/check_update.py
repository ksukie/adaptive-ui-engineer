#!/usr/bin/env python3
"""Rate-limit Adaptive UI Engineer update checks and format reminder metadata."""
...
REMOTE_RELEASE_URL = (
    ...
    "plugins/adaptive-ui-engineer/skills/adaptive-ui-s/release.json"
)
UPDATE_GUIDE_URL = "https://github.com/ksukie/adaptive-ui-engineer#update"
...
INVOCATION_RE = re.compile(
    r"(?<![A-Za-z0-9_-])(?:\$adaptive-ui-(?:s|n)|@Adaptive-UI-(?:S|N))(?![A-Za-z0-9_-])",
    flags=re.IGNORECASE,
)
...
def fetch_release(url: str = REMOTE_RELEASE_URL) -> dict[str, Any]:
    request = urllib.request.Request(
        ...
        headers={
            ...
            "User-Agent": "adaptive-ui-engineer-update-check/1.1.0",
        },
    )
...
def resolve_state_path(explicit_directory: str | None = None) -> Path:
    ...
    if os.name == "nt" and os.environ.get("LOCALAPPDATA"):
        return Path(os.environ["LOCALAPPDATA"]) / "adaptive-ui-engineer" / STATE_FILE_NAME
    state_home = os.environ.get("XDG_STATE_HOME")
    root = Path(state_home).expanduser() if state_home else Path.home() / ".local" / "state"
    return root / "adaptive-ui-engineer" / STATE_FILE_NAME
...
def hook_context(result: dict[str, Any]) -> str:
    ...
    return (
        ...
        "a clearly separated Adaptive UI Engineer update notice using the display-only JSON below. "
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Check Adaptive UI Engineer release metadata")
